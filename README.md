...
> **检索清单是主输出，3×3 交叉矩阵是综合速览。表格是地图，深挖是报告。**
...
- **六维检索清单**（核心输出）：完整的三时三事搜索清单——每个维度含专属字段（检索重点、关键词建议 [附设计理由]、识别目标、更新频率等），可直接用于搜索引擎和学术数据库
- **3×3 交叉矩阵速览**：在检索清单之上的综合层——9 个交叉点揭示平铺列表看不到的"时间 × 层次"关联
...
输入"欧盟碳边境调节机制"后，六维检索清单中的一维（现状时间）：
