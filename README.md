...
进入一个新研究选题时，研究者面临的最大障碍不是"信息不足"，而是**不知道自己不知道什么**（乔哈里窗的"未知区"）。IR Chronicle Matrix 的多阶段交互设计，在每一步对话中系统性缩小四个认知盲区：

```
          我已知              我未知
         ┌─────────┬─────────┐
  AI     │ 开放区   │ 盲区     │  ← 六维检索清单覆盖你不知道去查的维度
  已知   │ (扩大它) │ (发现它) │     关键词设计理由让"为什么查"透明化
         ├─────────┼─────────┤
  AI     │ 隐藏区   │ 未知区   │  ← 认知边界检查表 + 反事实追问 + 隐藏前提探测
  未知   │ (显性化) │ (探测它) │     专门设计来探测"我们都没想到去想"的领域
         └─────────┴─────────┘
```

| 象限 | 本 skill 的破除机制 |
|------|------------------|
| **开放区**（已知已知） | Step 1 边界界定 + 已知认知亮底，迫使脑中零散认知显性化 |
| **盲区**（已知未知） | 六维检索清单每关键词附设计理由；常识性叙述风险提示标注"最容易被重复但未必被检验的叙事" |
| **隐藏区**（未知已知） | 深挖选择环节要求主动选出维度——为什么选这个不选那个？隐性学术直觉变为可审视的显性判断 |
| **未知区**（未知未知） | 5 维认知边界检查表（时序错觉/层次错位/行为者缺失/叙事捕获/类比陷阱）+ 反事实追问暴露默认因果假设 |

**核心纪律**：每一步必须在扩大共识区之后才进入下一步。边界未确认 → 不生成清单。深挖方向未选定 → 不做深挖。Phase 1 未完成 → 不触发 Phase 2。AI 不代替研究者判断。
